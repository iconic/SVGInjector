...
(function (window, document) {
  ...
  var queueRequest = function(requestQueue, callback, fileName, fragId, state) {
    requestQueue[fileName] = requestQueue[fileName] || [];
    requestQueue[fileName].push({callback:callback, fragmentId:fragId, state:state});
  };

  var processRequestQueue = function(svgCache, config, requestQueue, url, onElementInjectedCallback, injections, el, ranScripts) {
    for (var i = 0, len = requestQueue[url].length; i < len; i++) {
      // Make these calls async so we avoid blocking the page/renderer
      /* jshint loopfunc: true */
      (function (index) {
        setTimeout(function () {
          requestQueue[url][index].callback(cloneSvg(config, svgCache[url], requestQueue[url][index].fragmentId), requestQueue[url][index].state);
        }, 0);
      })(i);
  ...
  var loadSvg = function(onElementInjectedCallback, injections, env, svgCache, requestQueue, config, url, el, ranScripts, onLoadCompleteCb) {
    ...
    if (svgCache[fileName] !== undefined) {
      if (svgCache[fileName] instanceof SVGSVGElement) {
        ...
        onLoadCompleteCb(cloneSvg(config, svgCache[fileName], fragId), state);
      }
      else {
        // We don't have it in cache yet, but we are loading it, so queue this request
        queueRequest(requestQueue, onLoadCompleteCb, fileName, fragId, state);
      }
    }
    else {
      ...
      httpRequest.onreadystatechange = function () {
        ...
        if (httpRequest.readyState === 4) {
          ...
          if (httpRequest.status === 404 || httpRequest.responseXML === null) {
            onLoadCompleteCb('Unable to load SVG file: ' + fileName);

            if (env.isLocal) onLoadCompleteCb('Note: SVG injection ajax calls do not work locally without adjusting security setting in your browser. Or consider using a local webserver.');
          ...
          if (httpRequest.status === 200 || (env.isLocal && httpRequest.status === 0)) {
            ...
            // :NOTE: IE8 and older doesn't have DOMParser, but they can't do SVG either, so...
              ...
              if (!xmlDoc || xmlDoc.getElementsByTagName('parsererror').length) {
                onLoadCompleteCb('Unable to parse SVG file: ' + url);
                return false;
              }
              else {
                // Cache it
            ...
            //svgCache, config, requestQueue, url, onElementInjectedCallback, injections, el
            processRequestQueue(svgCache, config, requestQueue, fileName, onElementInjectedCallback, injections, el, ranScripts);
          }
          else {
  ...
  var onLoadSVG = function(svg, state){
    if (typeof svg === 'undefined' || typeof svg === 'string') {
      state.callback(svg);
      return false;
    }

    var imgId = state.el.getAttribute('id');
    if (imgId) {
      svg.setAttribute('id', imgId);
    }

    var imgTitle = state.el.getAttribute('title');
    if (imgTitle) {
      svg.setAttribute('title', imgTitle);
    }

    // Concat the SVG classes + 'injected-svg' + the img classes
    var classMerge = [].concat(svg.getAttribute('class') || [], 'injected-svg', state.el.getAttribute('class') || []).join(' ');
    svg.setAttribute('class', uniqueClasses(classMerge));

    var imgStyle = state.el.getAttribute('style');
    if (imgStyle) {
      svg.setAttribute('style', imgStyle);
    }

    // Copy all the data elements to the svg
    var imgData = [].filter.call(state.el.attributes, function (at) {
    ...
    var presARAttr = state.el.getAttribute('preserveAspectRatio');
    if(presARAttr){
    ...
    prefixIdReferences(svg, state.injections.count);
    ...
    if (scriptsToEval.length > 0 && (config.evalScripts === 'always' || (config.evalScripts === 'once' && ! state.ranScripts[state.url]))) {
      ...
      state.ranScripts[state.url] = true;
    }
    ...
    forEach.call(styleTags, function (styleTag) {
      ...
      if ((svgClassList.indexOf(config.removeStylesClass)>=0 || config.removeAllStyles) && (svgClassList.indexOf(config.keepStylesClass)<0) ) {
        ...
        if(config.prefixStyleTags){
          doPrefixStyleTags(styleTag, state.injections.count, svg);
        }
        else{
          // :WORKAROUND:
    ...
    state.el.parentNode.replaceChild(svg, state.el);

    // Now that we no longer need it, drop references
    // to the original element so it can be GC'd
    delete state.injections.elements[state.injections.elements.indexOf(state.el)];
    //el = null;

    // Increment the injected count
    state.injections.count++;

    state.onElementInjectedCallback(svg);
  };
   ...
   */
  var SVGInjector = function (options) {
    ...
    this.env = {};
    this.env.isLocal = window.location.protocol === 'file:';
    this.env.hasSvgSupport = document.implementation.hasFeature('http://www.w3.org/TR/SVG11/feature#BasicStructure', '1.1');
    ...
    this.config.evalScripts = options.evalScripts || 'always';

    // Location of fallback pngs, if desired
    this.config.pngFallback = options.pngFallback || false;

    // Only inject the part of the svg, that is specified
    // as visible through the id of an SVGViewElement
    // is default mode
    this.config.onlyInjectVisiblePart = options.onlyInjectVisiblePart || true;

    this.config.keepStylesClass  = (typeof options.keepStylesClass === 'undefined') ?
      '' : options.keepStylesClass;

    this.config.spriteClassName  = (typeof options.spriteClassName === 'undefined') ?
      defaultSpriteClassName : options.spriteClassName;

    this.config.spriteClassIdName  = (typeof options.spriteClassIdName === 'undefined') ?
      defaultSpriteClassIdName : options.spriteClassIdName;

    this.config.removeStylesClass = (typeof options.removeStylesClass === 'undefined') ?
      defaultRemoveStylesClassName : options.removeStylesClass;

    this.config.removeAllStyles = (typeof options.removeAllStyles === 'undefined') ?
      false : options.removeAllStyles;

    this.config.fallbackClassName = (typeof options.fallbackClassName === 'undefined') ?
      defaultFallbackClassNames : options.fallbackClassName;

    this.config.prefixStyleTags  = (typeof options.prefixStyleTags === 'undefined') ?
      true : options.prefixStyleTags;

    this.config.spritesheetURL = (typeof options.spritesheetURL === 'undefined' || options.spritesheetURL === '') ?
      false : options.spritesheetURL;

    this.config.prefixFragIdClass = spriteClassIdName;

    this.config.forceFallbacks = (typeof options.forceFallbacks === 'undefined') ?
      false : options.forceFallbacks;

    if(this.config.forceFallbacks){
      this.env.hasSvgSupport = false;
    }

    replaceNoSVGClass(document.querySelector('html'), 'no-svg', this.env.hasSvgSupport);

    if(this.env.hasSvgSupport && typeof options.removeStylesClass === 'undefined' ){ // user does not want to use his own custom class -> write this style tag
      writeDefaultClass(this.config.removeStylesClass);
    }
  ...
  SVGInjector.prototype.injectElement = function (el, onElementInjectedCallback) {
    ...
    if(this.config.spritesheetURL === false){
      // Grab the src or data-src attribute
      imgUrl = el.getAttribute('data-src') || el.getAttribute('src');
    }
    else{
      imgUrl = this.config.spritesheetURL + '#' + getSpriteIdFromClass(el);
    ...
    if (this.env.hasSvgSupport) {
      if(isArray(this.config.fallbackClassName)) {
        removeFallbackClassNames(el, imgUrlSplitByFId[1], this.config.fallbackClassName);
      }
    }
    else{
      ...
      // Global PNG fallback directoriy defined, use the same-named PNG
      else if (this.config.pngFallback) {
        ...
        if(isArray(this.config.fallbackClassName)) {
          setFallbackClassNames(el, imgUrlSplitByFId[1], this.config.fallbackClassName);
        }
        else if(isFunction(this.config.fallbackClassName)) {
          console.info('custom function to create fallbackClassName');
          this.config.fallbackClassName(el, imgUrlSplitByFId[1]);
        }
        else if(typeof this.config.fallbackClassName === 'string') {
          svgElemSetClassName(el, this.config.fallbackClassName);
        }
        else{
    ...
    if (this.injections.elements.indexOf(el) !== -1) {
    ...
    this.injections.elements.push(el);

    // Try to avoid loading the orginal image src if possible.
    el.setAttribute('src', '');

    // Load it up
    loadSvg(onElementInjectedCallback, this.injections, this.env, this.svgCache, this.requestQueue, this.config, imgUrl, el, this.ranScripts, onLoadSVG);
  };
