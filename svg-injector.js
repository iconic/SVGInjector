...
((window, {implementation}) => {
  ...
  const hasSvgSupport = implementation.hasFeature('http://www.w3.org/TR/SVG11/feature#BasicStructure', '1.1');
  ...
  const injectElement = (el, evalScripts, pngFallback, callback) => {
    ...
    if (injectedElements.includes(el)) {
      return;
    }
    ...
    loadSvg(imgUrl, svg => {
      ...
      const imgData = [].filter.call(el.attributes, ({name}) => (/^data-\w[\w\-]*$/).test(name) || 'onclick'.match(name));
